...
var Histogram = stats.Histogram = function Histogram(params) {
    ...
    this.params = params;
    if (params) {
      this.size = params.buckets || 3000;
      this.percentiles = params && params.percentiles || [0.95, 0.99];
      this.percentileFormat = params.percentileFormat || '';
    }
    this.clear();
};
Histogram.prototype =  {
    ...
    summary: function() {
        ...
        self.percentiles.forEach(function(percentile) {
            var key = percentile * 100;
            if (self.percentileFormat === 'prefixed') {
              key = 'p' + key.toString().replace(/\./, '');
            } else {
              key = key + '%';
            }
            s[key] = self.percentile(percentile);
        });
        return s;
    },
